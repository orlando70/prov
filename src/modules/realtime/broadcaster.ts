...
export async function setupBroadcaster() {
  await redisSubscriber.psubscribe('match:*:events');
  logger.info('Subscribed to match events pattern');
    ...
    try {
      ...
      switch (payload.kind) {
        ...
        case 'EVENT':
          ...
          break;
        case 'STATS':
          io.to(`match:${matchId}`).emit('match:stats_update', {
            matchId,
